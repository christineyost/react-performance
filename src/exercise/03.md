...
- `React.memo()` can be used to prevent re-renders to components unless their
  props change
- The second argument to `React.memo()` is a custom comparator that should
  return `false` if a component should be re-rendered and `true` if not
- These custom comparators can be tedious to write and hard to get right, so a
  better way is to pass primative values as props, then React can automaticatlly
  detect if a component should be re-rendered or not (see final state of this
  exercise)
...
It's much better to use `React.memo` more intentionally and further, there are
other things you can do to reduce the amount of unnecessary re-renders
throughout your application (which we'll talk about more later).

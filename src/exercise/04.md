...
- Rendering huge lists definitely leads to performance problems in react. In
  this example we're rendering the entire list of 16,000+ cities.
- There are some existing libraries that can help with rendering large lists,
  `react-virtual` is one such library. See exercise for an example of how to use
  this library.
- The result is only a few list items are rendered instead of 16,000+ and the
  text in them simply changes as the user scrolls.

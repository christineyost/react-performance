...
- `React.useMemo()` can be used to memoize values that are expensive to
  calculate. They will only be re-calculated if anything in the dependencies
  array argurment for this hook is changed
- For really expensive calculations, web workers can be used to run those
  calculations asynchronously in order to avoid tying up the main browser thread
- This asynchronous work needs to happen in `React.useEffect()` because this is
  the hook that should be used for asynchronus work

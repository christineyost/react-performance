...
- The `React.Profiler` component can be used to monitor performance of
  components in production over time. That way if a component starts having
  performance issues we should be able to find out
- The trace from `scheduler/tracing` can be used to monitor individual
  interactions (button clicks, form submits, etc.) and their timings

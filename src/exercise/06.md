...
- Colocating state can help a lot with performance if too much state is being
  managed globally
- If the state actually does need to be global for whatever reason, instead of
  having just one context that manages all global state context, that can be
  broken up so that contexts are specific to what they are managing. In this
  case, the DogContext can be separated from the GridStateContext
- If there's a component that only needs a small piece of state, performance can
  improve by creating a pass-through component. This new component will get the
  piece of state needed by the underlying component and pass it through. That
  way if the piece of state did not change, the entire component will not be
  re-rendered
- A nicer way to do what's just above here is use a Higher Order Component which
  can be reused
- Recoil is a tool that can be very useful for solving problems like this
  specific one (this is a very specific problem though)

...
- React has a feature which allows lazy loading of expensive modules. Those
  modules should export a React component by default
- Components that are lazy loaded should be wrapped by the `<React.Suspense>`
  component which has a fallback UI. The `<React.Suspense>` component should be
  kept close to the component that is loaded lazily
- There are some magic webpack comments that can be used to eagerly load modules

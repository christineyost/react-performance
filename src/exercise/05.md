...
- Even if components are wrapped in `React.memo()`, they will still re-render if
  a context value that they use changes. Context value is an array, and since
  React does a shallow equality check, the arrays are never equal
- To fix this problem, we can use `React.useMemo()` to memoize the context value
  itself
- Another problem might be that some components use only part of the value
  provided by a context provider. If that's the case it can help to split the
  provider into two providers - one to provide the state and one to provide the
  dispatch function. Then if a component only uses the dispatch function, it
  won't need to be re-rendered everytime the state changes
